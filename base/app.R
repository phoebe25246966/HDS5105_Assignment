    ...
    tabItems(
      ...
      tabItem(
        ...
        fluidRow(
          ...
          box(
            width       = 6,
            title       = "Logistic regression (SARA)",
            status      = "info",
            solidHeader = TRUE,
            collapsible = TRUE,
...
server <- function(input, output) {
  ...
  filtered_data <- reactive({
    ...
    dat %>%
      filter(
        AGE >= input$filter_age[1],
        AGE <= input$filter_age[2]
      )
  })
  
  # SARA: Risk factor explorer
  risk_data <- reactive({
    dat <- filtered_data()
